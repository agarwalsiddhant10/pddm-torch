...
def run_job(args, save_dir=None):
    ...
    dyn_models = Dyn_Model(inputSize, outputSize, acSize, reg=[2, 20], params=args)
    ...
    while counter < num_iters:
        ...
        plot_histograms(save_dir, counter, rollouts_trainRand, rollouts_trainOnPol, rollout_info_gt)
        ...
        if counter%5 == 0:
            ...
            ax.legend(loc='best')
            fig.suptitle('Optimal Rewards Baoding GT vs Trained Model (Random + onpolicy)')
            ...
            for i in range(r):
                for j in range(c):
                    ...
                    #     fig.delaxes(ax[3, 3])
                    #     fig.delaxes(ax[3, 4])
                    #     fig.suptitle('Random States Cheetah GT vs Trained Model (Random + onpolicy)\n\n')
                    #     fig.tight_layout()
            ...
            fig.suptitle('Random States DClaw vs Trained Model (Random + onpolicy)\n\n')
            plt.tight_layout()
            plt.savefig(save_dir + '/states_rand/states_{}.png'.format(counter))
            plt.close()
            ...
            ax.set_ylabel('reward')
            ax.legend(loc='best')
            fig.suptitle('Random Rewards Baoding vs Trained Model (Random + onpolicy)')

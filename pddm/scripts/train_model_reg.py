...
def run_job(args, save_dir=None):
    ...
    dyn_models = Dyn_Model(inputSize, outputSize, acSize, reg = 9, params=args)

    mpc_rollout = MPCRollout(env, dyn_models, random_policy,
                                execute_sideRollouts, plot_sideRollouts, args)

    mpc_rollout_gt = MPCRollout(env, dyn_models, random_policy,
                                execute_sideRollouts, plot_sideRollouts, args)
    mpc_rollout_gt.use_ground_truth_dynamics=True
    mpc_rollout_gt.controller_mppi.use_ground_truth_dynamics = True
    ### init TF variables
    ...
    rollout_info_gt = None
    rollout_info_rand = None
    while counter < num_iters:
        ...
        if counter%5 == 0:
            if not os.path.exists(save_dir + '/states'):
                os.makedirs(save_dir + '/states')

            if not os.path.exists(save_dir + '/rewards'):
                os.makedirs(save_dir + '/rewards')

            if not os.path.exists(save_dir + '/states_rand'):
                os.makedirs(save_dir + '/states_rand')

            if not os.path.exists(save_dir + '/rewards_rand'):
                os.makedirs(save_dir + '/rewards_rand')

            if rollout_info_gt is None:
                starting_observation, starting_state = env.reset(return_start_state=True)

                rollout_info_gt = mpc_rollout_gt.perform_rollout(
                    starting_state,
                    starting_observation,
                    controller_type=args.controller_type,
                    take_exploratory_actions=False)

            if rollout_info_rand is None:
                rollout_info_rand = collect_random_rollouts(
                    env, random_policy, 1,
                    args.rand_rollout_length, dt_from_xml, args)
            ...
            obs = np.array(rollout_info_gt['observations'])
            acts = np.array(rollout_info_gt['actions'])

            state = obs[0]
            action = acts[0]

            next_state_mean, next_state_std = dyn_models.forward_model(state, action)

            model_states = []
            model_states_std = []
            model_states.append(np.copy(state))
            model_states_std.append(np.zeros_like(state))
            model_states.append(np.copy(next_state_mean))
            model_states_std.append(np.copy(next_state_std))


            #loop
            for i in range(1, acts.shape[0]):
                state = np.copy(next_state_mean)
                action = acts[i]
                next_state_mean, next_state_std = dyn_models.forward_model(obs[i], action)

                model_states.append(np.copy(next_state_mean))
                model_states_std.append(np.copy(next_state_std))

            model_states = np.array(model_states)
            model_states_std = np.array(model_states_std)
            obs_true = obs
            obs_pred = model_states
            print(model_states.shape)
            print('std: ', model_states_std.shape)
            print('pred: ', obs_pred.shape)
            # print(model_states_std)
            # model_states_std *=10
            print(np.mean(np.square(model_states - obs)))

            x = np.arange(obs_true.shape[0])
            r=4
            c=5
            ...
            for i in range(r):
                for j in range(c):
                    if c*i + j >= 18:
                        ...
                        fig.delaxes(ax[3, 3])
                        fig.delaxes(ax[3, 4])
                        fig.suptitle('Optimal States Cheetah GT vs Trained Model (Regression state 9)\n\n')
                        fig.tight_layout()
                        plt.savefig(save_dir + '/states/states_{}.png'.format(counter))
                        plt.close()
                        break
            ...
            #     rew_, _ = env.env.get_reward(obs_true[i+1], acts[i])
            #     rew_true.append(np.copy(rew_))
            #     rew_, _ = env.env.get_reward(obs_pred[i+1], acts[i])
            #     rew_pred.append(np.copy(rew_))

            # rew_true = np.array(rew_true)
            # rew_pred = np.array(rew_pred)
            # fig, ax = plt.subplots(nrows = 1, ncols =1, figsize=(24, 10))
            # ax.plot(x[1:], rew_true, color='blue', label='ground truth')
            # ax.plot(x[1:], rew_pred, color='red', label='predicted')
            # ax.set_xlabel('timestep')
            # ax.set_ylabel('reward')
            # ax.legend(loc='best')
            # fig.suptitle('Optimal Rewards DClaw GT vs Trained Model (Random + onpolicy)')
            # plt.savefig(save_dir + '/rewards/rewards_{}.png'.format(counter))
            # plt.close()
            ...
            ######################## Random trajectories ##############################
            # obs = np.array(rollout_info_rand['observations'])
            # acts = np.array(rollout_info_rand['actions'])
            obs = rollout_info_rand[0].states
            acts = rollout_info_rand[0].actions[:-1]
            ...
            for i in range(r):
                for j in range(c):
                    if c*i + j >= 18:
                        ...
                        fig.delaxes(ax[3, 3])
                        fig.delaxes(ax[3, 4])
                        fig.suptitle('Random States Cheetah GT vs Trained Model (Regression state 9)\n\n')
                        fig.tight_layout()
                        plt.savefig(save_dir + '/states_rand/states_{}.png'.format(counter))
                        plt.close()
                        break
            ...
            # fig.suptitle('Random States DClaw GT vs Trained Model (Random + onpolicy)\n\n')
            # plt.tight_layout()
            # plt.savefig(save_dir + '/states_rand/states_{}.png'.format(counter))
            # plt.close()
            ...
            # ax.set_ylabel('reward')
            # ax.legend(loc='best')
            # fig.suptitle('Random Rewards DClaw GT vs Trained Model (Random + onpolicy)')
            # plt.savefig(save_dir + '/rewards_rand/rewards_{}.png'.format(counter))
            # plt.close()
